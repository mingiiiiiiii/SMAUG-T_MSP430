...
void crypto_kem_keypair(uint8_t *pk, uint8_t *sk) {
	...
	randombytes_aes(sk + PKE_SECRETKEY_BYTES, T_BYTES);
	for (int i = 0; i < PUBLICKEY_BYTES; i++) sk[i + PKE_SECRETKEY_BYTES + T_BYTES] = pk[i];
}
...
int crypto_kem_enc(uint8_t *ctxt, uint8_t *ss, const uint8_t *pk) {
	...
	randombytes_aes(mu, DELTA_BYTES);
	hash_h(buf, pk, PUBLICKEY_BYTES);
	hash_g(buf, DELTA_BYTES + CRYPTO_BYTES, mu, DELTA_BYTES, buf, SHA3_256_HashSize);
...
int crypto_kem_dec(uint8_t *ss, const uint8_t *ctxt, const uint8_t *sk) {
	...
	hash_g(buf, DELTA_BYTES + CRYPTO_BYTES, mu, DELTA_BYTES, hash_res, SHA3_256_HashSize);
	...
	hash_g(buf_tmp, DELTA_BYTES + CRYPTO_BYTES, sk + 2 * MODULE_RANK + SKPOLYVEC_BYTES, T_BYTES, hash_res,
				 SHA3_256_HashSize);
