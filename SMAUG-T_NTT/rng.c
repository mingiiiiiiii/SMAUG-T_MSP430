...
void randombytes_aes_init(unsigned char *entropy_input, unsigned char *personalization_string, int security_strength) {
...
int randombytes_aes(unsigned char *x, unsigned long long xlen) {
	...
	if (!DRBG_ctx.init) {
		...
		randombytes_aes_init(entropy_input, NULL, 256);
	}
